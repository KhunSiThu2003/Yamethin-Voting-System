<template>
  ...
  <div
    v-if="currentUser"
    class="min-h-screen bg-gray-100 pt-24 pb-10 text-center flex flex-col justify-center items-center text-gray-900 rounded-lg dark:bg-gray-900 dark:text-gray-200"
  >
    <div>
      ...
      <h1 class="text-blue-500 text-2xl font-bold mb-10 dark:text-blue-400">
        University Election
      </h1>
    </div>

    <div v-if="!votingEnd" class="p-2 border rounded border-gray-300 dark:border-gray-700">
      <h1 class="text-sm mb-2 opacity-50 dark:text-gray-300 text-center">
        Voting Ends In
      </h1>
      <div class="flex justify-center gap-4 text-sm sm:text-sm md:text-base items-center">
        <div class="flex flex-col items-center text-yellow-300">
          <span id="days" class="text-sm font-bold">{{ dayString }}</span>
          <span class="text-xs">Days</span>
        </div>
        <div class="flex flex-col items-center text-blue-500">
          <span id="hours" class="text-sm font-bold">{{ hourString }}</span>
          <span class="text-xs">Hours</span>
        </div>
        <div class="flex flex-col items-center text-green-400">
          <span id="minutes" class="text-sm font-bold">{{ minString }}</span>
          <span class="text-xs">Minutes</span>
        </div>
        <div class="flex flex-col items-center text-red-600">
          <span id="seconds" class="text-sm font-bold">{{ secString }}</span>
          <span class="text-xs">Seconds</span>
        </div>
      </div>
    </div>

    <div class="flex w-full md:w-1/2 mx-auto justify-evenly items-center md:space-x-8 pt-14">
      <!-- University King Card -->
      <div
...
export default {
  ...
  setup() {
    ...
    const { dayString, hourString, minString, secString, updateCountdown, votingEnd } =
      deadLine("university");
    const { votingEnd: majorVotingEnded } = deadLine("major");
    ...
    const checkVotingStatus = (candidateId) => {
      if (votingEnd.value) {
        showVotingEndedAlert();
      } else if (!majorVotingEnded.value) {
        showMajorVotingNotEndedAlert();
      } else {
        router.push({ name: 'universitycandidates', params: { id: candidateId } });
      }
    };
    ...
    const showMajorVotingNotEndedAlert = () => {
      Swal.fire({
        title: 'Major Voting Still Ongoing',
        html: `
          <div class="text-center">
            <p class="text-gray-600 dark:text-gray-300">University voting will open after major voting has ended.</p>
            <p class="text-gray-600 dark:text-gray-300 mt-2">Please complete your major voting first.</p>
          </div>
        `,
        icon: 'warning',
